import { Box, BoxProps, Center, CircularProgress, Divider, Heading, Table, Tbody, Td, Th, Thead, Tr } from '@chakra-ui/react'
...
import { clearTracks, fetchTracks, setTrackSourceId } from 'state/Tracks/actions'
...
interface TrackListViewProps extends BoxProps, RouteComponentProps<{id:string}> {

}

export const TrackListView: React.FC<TrackListViewProps> = ({match}) => {
    
    const tracks = useSelector((state:RootState)=> state.tracks.tracks )
    const offsets = useSelector((state:RootState)=>state.offsets)
    const playlistSource = useSelector((state:RootState)=>state.tracks.playlistSource)
    const dispatch = useDispatch()
    

    useEffect(()=>{
  
        dispatch(setTrackSourceId('playlists',match.params.id))
    ...
    return (
        <>
      <Heading variant='md' m='0.8rem' ml='2rem'>{playlistSource?.name}</Heading>
