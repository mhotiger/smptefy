...
const initialState: MidiPlayerState = {
	player: new MidiTcPlayer({ h: 0, m: 0, s: 0, f: 0 }, fps30, {
		h: 0,
		m: 0,
		s: 0,
		f: 0,
...
export const midiPlayerReducer = produce(
	(state: MidiPlayerState, action: MidiAction): MidiPlayerState => {
		...
		switch (action.type) {
			...
			case SET_MIDI_INPUT:
				if (action.input < state.player.inputs.length) {
					state.player.activeInput =
						state.player.inputs[action.input];
					state.player.activeInputIndex = action.input;
				}

				return state;

			case SET_MIDI_OUTPUT:
				if (action.output < state.player.outputs.length) {
					state.player.activeOutput =
