...
export const userFetchEpic: Epic<AllActions, AllActions, RootState, void> = (
	action$,
	state$
) => {
	return action$.pipe(
...
const userLogoutEpic: Epic<AllActions, AllActions, RootState, void> = (
	action$,
	state$
) => {
	return action$.pipe(
...
const fetchSpotifyUserEpic: Epic<AllActions, AllActions, RootState, void> = (
	action$,
	state$
) => {
	return action$.pipe(
		...
		mergeMap((action) => {
			return defer(() =>
				ajax.getJSON<SpotifyApi.UserObjectPrivate>(
				...
				map((response: SpotifyApi.UserObjectPrivate) => {
					...
					if (response.product !== 'premium') {
						return setError(
							'You need a spotify premuim account to use smptefy'
						);
					}
